/* Блок: Глобальный объект uw и его свойства */
let uw = {
  document: $(document),
  window: $(window),
  htmlPages: {
...
/* Блок: Утилита для асинхронной загрузки JS файлов */
...
uw.utils.isStaticExport = () => {
  const host = window.location.hostname;
  return host !== 'unitedofweb.com' && host !== 'www.unitedofweb.com';
};
...
/* Блок: Обработчики кликов по формам и элементам aform */
...
/* Блок: Обработчик SPA (Single Page Application) маршрутизации (eventClickSpa) */
...
uw.executePageExtended = function() {
  ...
  $('.custom-checkbox').each((idx, item) => {
    ...
    $(item).html($(item).text() + '<input class="hide" id="custom-checkbox-input-' + $(item).attr('data-name') +
      idx + '" type="checkbox" name="' + $(item).attr('data-name') + '">')
  });
  ...
  $(".custom-checkbox").click(function() {
    ...
    if ($checkBox.hasClass('active')) {
      ...
      $('#custom-checkbox-input-' + $checkBox.attr('data-name') + $checkBox.attr('data-idx')).prop("checked",
        false);
    } else {
...
uw.loadAsyncForm = function() {
  ...
  $('form').each(async function(index) {
    if ($(this).attr('type') === "save-update") {
      let generateId = await uw.utils.sha256(new Date().getTime() + uw.utils.getRandomInt(-999999999,
        999999999));
...
/* Блок: Загрузка страниц через AJAX (SPA) */
uw.showPage = function($page, $isRepeat) {
  //if((($page === 'index') ? '/' : '/' + $page) == location.pathname && $isRepeat !== true)
  //    return false;

  document.dispatchEvent(new CustomEvent("uwBlockPageOnStartLoad", {
    "detail": {
      page: $page,
      isRepeat: $isRepeat
    }
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data == 'refresh') {
        document.dispatchEvent(new CustomEvent("uwBlockPageOnFinishLoad", {
          "detail": {
            reload: true,
            page: $page,
            title: $('title').text(),
    ...
    error: function(jqXHR, textStatus, errorThrown) {
      ...
      document.dispatchEvent(new CustomEvent("uwBlockPageOnFinishLoad", {
        "detail": {
          errorMessages: [jqXHR, textStatus, errorThrown],
          page: $page,
          title: spaTitle,
          isRepeat: $isRepeat
        }
...
uw.files.uploadFileConvert = (file) => {
  if (file.type.includes('image')) {
    heic2any({
      ...
      uw.files.uploadFile(new File([blob], file.name + ".png", {
        type: "image/png"
      }));
    }, error => {
    ...
    success: function(respond, textStatus, jqXHR) {
      ...
      if (typeof respond.error === 'undefined') {
        ...
        if (typeof result.success === 'undefined')
          M.toast({
            html: 'Картинка успешно загружена',
            classes: 'rounded'
          });
        else
          M.toast({
        ...
        if ($catId === -1) {
          $('#iphone-body').css('background', 'url("/upload/iphone/exclusive/' + result.success.files.files[0] +
            '") center no-repeat');
        ...
        M.toast({
          html: 'Ошибка сервера: ' + respond.error,
          classes: 'rounded'
        });
      }
    },
    ...
    $(".media-droppable").droppable({
      ...
      drop: function(event, ui) {
        ...
        $.ajax({
          ...
          url: '/ajax.php' + window.location.search,
          dataType: 'json',
          data: 'ajax=true&action=media-move&mid=' + encodeURIComponent(ui.draggable.attr('attr-id')) +
            '&dir=' + encodeURIComponent($(this).attr('attr-dir')),
          success: function(data) {
            console.log(data);
            ui.draggable.remove();
          },
          error: function(jqXHR, textStatus, errorThrown) {
            ...
            M.toast({
              html: 'We have some error, please try again',
              classes: 'rounded'
            });
          }
        });
