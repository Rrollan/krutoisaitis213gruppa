...
uwTpl.hidePreloader = () => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      $('#loading-grid-container').fadeOut('slow'); // Явно скрываем сетку, если она вне preloader-page
    }, 1000);
  }, showDelay)
};
...
uwTpl.showMainAnimation = function() {
  // Инициализация бегущей строки для портфолио или баннеров
  try {
    $('.marquee').marquee({
      duration: 15000,
      startVisible: true,
  ...
  // Инициализация частиц для фоновых эффектов в портфолио
  try {
    Particles.init({
...
uwTpl.initTemplate = (e) => {
  ...
  // Глобальный обработчик для открытия карточек в полный размер
  $('body').off('click', 'img').on('click', 'img', function(e) {
    // Проверяем, находится ли изображение внутри карточки
    var $img = $(this);
    var $card = $img.closest('.card');
    
    // Если это изображение карточки или lazyload изображение
    if ($card.length || $img.hasClass('lazyload') || $img.closest('.card').length) {
      ...
      if (src && src !== '' && !src.includes('undefined')) {
        ...
        // Скрываем элементы preloader'а и overlay
        var $loaderElements = $('#loading-grid-container, .loader-text-container, .l-shape, .loading-overlay');
        $loaderElements.css('visibility', 'hidden');
        $loaderElements.css('pointer-events', 'none');
        
        // Создаем модальное окно
        var modalHtml = '<div id="image-modal" style="display: flex; align-items: center; justify-content: center; z-index: 9999999; position: fixed; top: 0; left: 0; width: 100%; height: 100vh; background: rgba(0, 0, 0, 0.95); margin: 0; padding: 0; overflow: hidden; cursor: pointer;"><img src="' + src + '" style="max-width: 90%; max-height: 90%; width: auto; height: auto; object-fit: contain; display: block; cursor: pointer;" /></div>';
        ...
        var closeModal = function() {
          $modal.fadeOut(200, function() {
            $modal.remove();
            $loaderElements.css('visibility', 'visible');
            $loaderElements.css('pointer-events', 'auto');
        ...
        // Закрытие по Escape
        $(document).on('keydown.imageModal', function(evt) {
          if (evt.keyCode === 27) {
            closeModal();
          }
        });
  ...
  // Инициализация бегущей строки для портфолио или других элементов
  try {
    const phrases = [
      "Web Development",
      "High Level Design",
  ...
  setTimeout(() => {
    $('#postload').show();
    $('#preloader-page, #loading-grid-container').fadeOut('slow'); // Скрываем и страницу прелоадера, и сетку
  }, 2500);
}
...
Element.prototype.fadeIn = function(speed) {
  ...
  ''; // Reset display property to default or you can set to 'block' or 'inline', depending on your needs
